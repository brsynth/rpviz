...
# The 'svg' field holds RDKit-rendered 2D depictions whose exact path
# coordinates vary between RDKit versions. Exclude it from comparisons so the
# tests validate structure/data rather than version-specific rendering.
EXCLUDE_SVG = [r"\['svg'\]"]
...
def __read_and_sort(path: str, skip=[]) -> list:
    with open(path, encoding="utf-8") as fh:
        ...
        for line in fh.readlines():
            if not any(skip_item in line for skip_item in skip):
...
def test_dir_input(mocker, tmpdir):
    ...
    assert not deepdiff.DeepDiff(
        ...
        test_objects,
        ignore_order=True,
        exclude_regex_paths=EXCLUDE_SVG,
    )
