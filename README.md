...
## Install
```sh
conda install -c conda-forge rpviz
```
...
```sh
python -m rpviz <input-folder> <output-folder>
```
...
python -m rpviz sample/input/as_dir sample/output/as_dir
```

Using a tar file as input:
```sh
conda activate <myenv>
python -m rpviz sample/input/as_tar.tgz sample/output/as_tar
```
