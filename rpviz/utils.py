...
from rplibs.cobra_format import uncobraize
...
def _specie_is_sink_old(
    ...
    :rtype: bool
    """
    if not _specie_is_target_old(specie_id) and not _specie_is_intermediate_old(
        specie_id
    ):
        return True
    return False
...
def _get_reaction_ecs(rxn_dict: dict) -> list:
    if "ec-code" in rxn_dict["miriam"] and len(rxn_dict["miriam"]["ec-code"]):
...
def _get_specie_node_id_old(specie_dict: dict, specie_id: str = None) -> str:
    ...
    elif "metanetx" in specie_dict["miriam"] and len(specie_dict["miriam"]["metanetx"]):
        sorted_mnx_ids = sorted(
            specie_dict["miriam"]["metanetx"], key=lambda x: int(x.replace("MNXM", ""))
        )
        return sorted_mnx_ids[0]
    elif "chebi" in specie_dict["miriam"] and len(specie_dict["miriam"]["chebi"]):
        sorted_chebi_ids = sorted(
            specie_dict["miriam"]["chebi"], key=lambda x: int(x.replace("CHEBI:", ""))
        )
    ...
    if "miriam" in specie_dict:
        for db_name, db_id_list in specie_dict["miriam"].items():
            for db_id in db_id_list:
                ...
                xlinks.append({"db_name": db_name, "entity_id": db_id, "url": url_str})
    return xlinks
